...
(defun subscribe (channel prw)
  (funcall (car channel) prw))


(defparameter *channels* nil)

(defun add-channel (&key name)
  (let ((s 0))
    (setf (getf *channels* (read-from-string name)) s)))

;;;; Kinda works but work more!
;;; Decide between Ningle, Ninglex etc.

*channels*

(getf *channels* (read-from-string "vhg"))

(remf *channels* (read-from-string "vhg"))

(add-channel :name "VHG")
